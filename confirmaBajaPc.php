...
<div id="miVentana" style="position: fixed; width: 350px; height: 190px; top: 0; left: 0; font-family:Verdana, Arial, Helvetica, sans-serif; font-size: 12px; font-weight: normal; border: #333333 3px solid; background-color: #FAFAFA; color: #000000; display:none;">
 ...
 <p style="padding: 5px; text-align: justify; line-height:normal">
  ...
  <?php 
     $sql1=eliminarRamPc($idPc);
     $eliminaRam=mysql_query($sql1, $link);
     
     $sql=eliminarSoPc($idPc);
     $eliminaSo=mysql_query($sql, $link);
        
     $sql2=eliminarIncidentesPc($idPc);
     $eliminaIncid=mysql_query($sql2, $link);
        
        //si se pudo eliminar el registro de la tabla pc_ram, elimino la pc
        if($eliminaRam && $eliminaIncid) {
          $sql3=darBajaPc($idPc);
          $bajaPc=mysql_query($sql3, $link);          
        }
        
        if(!$eliminaRam || !$eliminaIncid || !$bajaPc) { 
           $sql4=nombrePc($idPc);
           $result=mysql_query($sql4, $link);
           $nombre=mysql_result($result,0);
        ?>
      <h3>La PC <?php echo $nombre ?> no pudo ser dad de baja. <br> Intente nuevamente</h3>
 ...
 ?>
    ...
    <h3>La baja se realizó correctamente</h3>
